...
export function AddonForm({
    ...
    useEffect(() => {
        ...
        // Formularz uzupełnia pola podczas edycji albo czyści je przy tworzeniu nowego dodatku
        setName(initialAddon?.name ?? '');
        setDescription(initialAddon?.description ?? '');
        setCateringCompanyId(initialAddon?.cateringCompanyId ?? companies[0]?.id ?? '');
        setIsActive(initialAddon?.isActive ?? true);
    }, [companies, initialAddon, open]);
            ...
            <Box component="form" onSubmit={handleSubmit}>
                ...
                <DialogContent>
                    <Stack spacing={2.5} sx={{ pt: 1 }}>
                        {/* Podstawowe dane dodatku widoczne w ofercie i menu */}
                        <TextField
                            label="Nazwa dodatku"
                            value={name}
                        ...
                        {/* Firma cateringowa określa właściciela dodatku i jego późniejsze użycie w menu */}
                        <TextField
                        ...
                        {/* Status aktywności pozwala ukryć dodatek przed klientami bez usuwania go z bazy */}
                        {canEditStatus && (
                            <FormControlLabel
                                control={
                                    <Switch
